...
public class ReadCoil {
    ...
    public  int readRegister(RTUTCPMasterConnection conn, int address, int quantity, int slaveId) {
        int data=0;
        try {
            conn.connect();
            ...
            System.out.println("寄存器数据获取失败（"+address+"）");
        } finally {
            conn.close();
        }
        return data;
    ...
    public  int readCoils(RTUTCPMasterConnection conn, int address, int quantity, int slaveId) {
        int data = 0;
        try {
            conn.connect();
            ...
            System.out.println("位状态数据获取失败（"+address+"）");
        } finally {
            conn.close();
        }
        return data;
